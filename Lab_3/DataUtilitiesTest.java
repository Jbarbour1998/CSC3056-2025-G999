...
public class DataUtilitiesTest {
    ...
    @Test
    public void testCalculateColumnTotalWithNullValue() {
        DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
        testValues.addValue(null, 0, 0); // Null value in row 0, column 0
        testValues.addValue(4.0, 1, 0);  // Normal value in row 1, column 0
        
        double result = DataUtilities.calculateColumnTotal(testValues, 0);
        assertEquals("Sum should ignore null value and sum only 4.0", 4.0, result, 0.0001);
    }
    ...
    public void testCalculateRowTotalWithNullValue() {
        DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
        testValues.addValue(null, 0, 0); // Null value in row 0, column 0
        testValues.addValue(5.0, 0, 1);  // Normal value in row 0, column 1
        
        double result = DataUtilities.calculateRowTotal(testValues, 0);
        
        // Only the non-null value should be summed (5.0)
        assertEquals("Sum should ignore null value and sum only 5.0", 5.0, result, 0.0001);
    }
    ...
    public void testGetCumulativePercentagesWithNullValue() {
        ...
        testValues.addValue("A", null); // Null value for key A
        testValues.addValue("B", 4.0);  // Normal value for key B
        
        KeyedValues result = DataUtilities.getCumulativePercentages(testValues);
        
        assertEquals("Cumulative percentage for B should be 1.0 since A is null", 1.0, result.getValue("B").doubleValue(), 0.0001);
    }
